...
池子只有在**里面真有一条连接**的时候才有用。进程起来后的第一个请求 —— 以及一天里第一次
问一个把自己的模型睡下去的服务 —— 要付握手和对方自己的冷启动；如果宿主在第一轮之前有一
个时刻（通话界面打开、会话刚建立），在那里花一个极小的请求，就等于把这两笔钱从第一轮回复
的关键路径上挪走。这条路由本身**按设计属于宿主代码**：这个包没有「通话」这种生命周期可以挂，
也只有宿主知道自己接了哪些供应商。客户端那一半不是宿主代码：`clients/call.js` 在拿到地址时
会在一通电话接通那一刻去请求它（`warmUrl`，见 [client.zh-CN.md](client.zh-CN.md)），
而 `examples/minimal_call/app.py` 用十来行实现了一条。

## 失败

供应商失败就抛。流水线在出错的那一片上接住它然后继续：一次合成失败会在 `done` 上留下一条
带片号的 `warning`，那句话的文本仍然在 `reply` 里。**一点音频都没出来**的片会先被再送一次
合成；重试之后仍然一个字都没出声的那几条消息，会列在 `done.unspoken` 里。模型调用失败则是
`error` 事件，因为已经没有回复可以继续了。
